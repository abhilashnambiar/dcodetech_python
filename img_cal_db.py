...
database = pymysql.connect(host="localhost",user="abhilash", password="abhilashn9")
...
def gotologin():
    def gotologgedin():
        def submit_data():
            database.select_db("py_db")
            cursor.execute("INSERT INTO user_data(email,dob,img,text) VALUES(%s,%s,%s,%s)",(l_email.get(),in_dob.get(),img_done,in_about.get(1.0,"end")))
            database.commit()
            print("Data added")
        ...
        def dwnld_img():
            ...
            with open(l_email.get()+"_"+current+".png","wb") as f:
                f.write(img_data[0])
